...
const RatingStar = ({ rating }) => {
  const [ratedStarArray, setRatedStarArray] = useState([]);
  const [emptyStarArray, setEmptyStarArray] = useState([]);

  useEffect(() => {
    const ratingArray = Array.from({ length: rating }, (v, i) => i + 1);
    const emptyRatingArray = Array.from(
      { length: 5 - rating },
      (v, i) => i + 1
    );
    setRatedStarArray(ratingArray);
    setEmptyStarArray(emptyRatingArray);
  }, [rating]);

  return (
    <div className="starContainer">
      {ratedStarArray.map((star) => (
        <svg
          key={star}
          className="star"
          viewBox="0 0 36 36"
